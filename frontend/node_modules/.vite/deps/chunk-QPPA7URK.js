...
var Engine = class {
  ...
  async load(params) {
    ...
    const { Container } = await import("./Container-3WEVN7AG.js"), id = params.id ?? (domSourceElement == null ? void 0 : domSourceElement.id) ?? `tsparticles${Math.floor(getRandom() * loadRandomFactor).toString()}`, { index, url } = params, options = url ? await getDataFromUrl({ fallback: params.options, url, index }) : params.options, currentOptions = itemFromSingleOrMultiple(options, index), { items } = this, oldIndex = items.findIndex((v) => v.id.description === id), newItem = new Container({
...
//# sourceMappingURL=chunk-QPPA7URK.js.map
